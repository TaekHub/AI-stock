...
class IndicatorChartWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (dates.isEmpty || selectedGroups.isEmpty) {
      print('지표 차트 데이터 오류: dates 또는 selectedGroups가 비어 있습니다.');
      return const Center(
        child: Text("선택된 지표가 없습니다.", style: TextStyle(color: Colors.white)),
      );
    }
    ...
    for (String group in selectedGroups) {
      ...
      if (!hasValidData) {
        print('지표 데이터 오류: $group에 유효한 데이터가 없습니다.');
        chartWidgets.add(
          Padding(
      ...
      for (String indicator in indicatorsInGroup) {
        ...
        if (spots.isEmpty) {
          print('지표 데이터 오류: $indicator에 유효한 스팟 데이터가 없습니다.');
          continue;
        }
      ...
      if (lineBarsData.isEmpty) {
        print('지표 차트 오류: $group에 표시할 데이터가 없습니다.');
        chartWidgets.add(
          Padding(
            padding: const EdgeInsets.all(12.0),
            child: Text(
      ...
      chartWidgets.add(
        Padding(
          padding: const EdgeInsets.symmetric(vertical: 16.0), // 간격 넓히기
          child: Card(
            color: Colors.grey[850], // 배경 색상 조정
            elevation: 6, // 그림자 강화
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
            child: Padding(
              padding: const EdgeInsets.all(20.0), // 내부 패딩 조정
              child: Column(
